...
### Inside a service — clean/hexagonal-lite (ADR-0010, amended by ADR-0011)

ADR-0006 decides the seams *between* services; **every service is built the same way inside**: three packages under `com.platinumcoin.pix.<service>` — `api/` (inbound adapters: controllers, request/response records, exception mapping), `domain/` (entities & value objects, **`usecase/`** — one class per inbound operation —, domain exceptions, and **ports** — the outbound interfaces the domain calls), and `infra/` (outbound adapters implementing those ports: DynamoDB/SQS/SNS/Redis/HTTP + Spring config, plus the composition root that instantiates the use cases).
...
**Every inbound operation is an explicit use case** (ADR-0011): a `<Verb><Noun>UseCase` class with a single `execute(...)`, named for the business intent, so `ls domain/usecase/` answers "what can this service do?". A controller therefore holds **no business policy** — it binds and bean-validates the wire shape, calls one use case, and maps the result (or a domain exception) to an HTTP status. Value normalization and generation, ownership guards, not-found decisions, limit rules and **reading the clock** all sit in the use case, which takes an injected `java.time.Clock` rather than calling `Instant.now()`. The same use case a controller calls is what the SQS consumers and scheduled jobs of Sprints 6–10 call, so no policy is reachable only through HTTP.

The rules are not aspirational — each service ships one **ArchUnit** `*ArchitectureTest` with two checks that fail the build: `domain/` imports nothing outward, and **`api/` never depends on an interface residing in `domain/`** (every port is an interface, every use case is a class — so a controller reaching a repository cannot be merged). `common-lib` is exempt: it *is* the shared adapter layer, so it legitimately depends on the web/servlet stack. The port boundary is also what lets ADR-0009's `labs/ledger-pg` restate the ledger's domain on PostgreSQL without touching the deployable — a **documented mirror, not a reuse**: `ledger-service` publishes a Boot fat jar, so depending on it is impossible without giving the deployable a second artifact purely to serve a lab, which is the coupling ADR-0009 forbids. Parity is asserted by one shared invariant suite rather than by the compiler (ADR-0009 amendment, step 50).
...
- Every service stateless → ≥3 replicas across 3 AZs behind load balancers; deploys are rolling with health checks.
  > **The one thing a second replica would need, and this build does not have.** The *request* paths are already replica-safe by construction — the debit is one conditional `TransactWriteItems`, the operation's identity is durable before the money moves (ADR-0014), finalization is fenced settle-XOR-reverse (ADR-0016), and every consumer dedupes by `eventId` — so N payment-services or N settlement-consumers are correct today. What is **not** solved is the **`@Scheduled` work**: the outbox publishers, the stuck-transaction scanner, the statement archiver and the alert evaluator would each run once *per replica*, with no leader election and no distributed lock anywhere in the repo. Most of that is merely wasteful rather than wrong (the publisher's mark-published is conditional, the archiver rewrites a month's object whole, a double-published event is deduped by the consumer) — but the alert evaluator would page N times per incident, and "wasteful but correct" is a claim that deserves a lock proving it rather than an argument. Single-instance locally, so it has never been exercised; the production shape is a lease (`ShedLock`-style) around each scheduled tick. Named here because §7.4 is where a reader will look for it. See also `notification-service`'s SSE registry, which is genuinely per-instance and says so.
...
- `POST /payments/pix` during the outage: the ledger call fails fast — **bounded timeouts, 2s to connect and 3s to read** (`payment-service` `application.yml`, `pix.ledger-service.*`), never a pinned request thread — → **`503 Service Unavailable` + `Retry-After: 5`**. Nothing was debited (the ledger write is the *first* money mutation), the idempotency record stores no successful response, so the client's retry with the same key processes cleanly. No inconsistency, no double charge — availability of *accepting new sends* degrades, correctness does not.
  > **What is *not* here, and it matters for this answer.** There is **no circuit breaker** in this build: every request during the outage pays the full timeout before its `503`, so a down ledger costs latency on the way to being refused, and the refusals keep hammering a struggling dependency. The production posture is the breaker — open after N consecutive failures, refuse *immediately*, half-open to probe — and the seam for it is the single outbound adapter (`HttpLedgerClient`) that every send already goes through. Called out rather than implied, because "fails fast" and "fails fast *and stops asking*" are different availability stories.
...
- **Timeout/failure ⇒ fail-open** (ADR-0005): the payment proceeds, flagged `FRAUD_SKIPPED`, and a `FraudCheckSkipped` event triggers asynchronous scoring; hits feed an alert/review path. **Failures are classified (ADR-0018, step 70):** a *transient* one (timeout, unreachable host, `5xx`, `429`) is `SKIPPED` at `WARN` and feeds the `fraud_fail_open_rate` ceiling; a *non-transient* one (`401`/`403`, any other `4xx`, an unreadable body, an adapter bug) is `FRAUD_ERROR` at `ERROR`, on its own series, raising `fraud_broken` on the first occurrence, and stamped durably on the transaction. Both proceed — the behaviour is deliberately identical, because a broken fraud deploy must not become a payments outage — but "fraud is struggling" and "fraud is off" are no longer the same sentence. Trade-off: fail-closed would let a fraud-service blip take down 100% of payments to stop a fraction of a percent of fraud — for a payments product, availability wins *at this layer*, with the risk bounded because limits still apply, and a production hybrid (fail-closed above a value threshold) is the documented evolution.
