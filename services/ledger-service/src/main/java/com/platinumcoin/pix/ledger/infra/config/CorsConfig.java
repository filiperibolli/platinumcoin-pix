 ...
 * from disk, hence {@code Origin: null} — can call ledger-service from the browser. Copies the
 * auth-service pattern verbatim (see that class for the full rationale).
 *
 ...
 * {@code OPTIONS} is short-circuited instead of being rejected with {@code 401}. That matters more
 * here than elsewhere: every endpoint of this service is authenticated, so without the ordering the
 * explorer could not reach a single one. Permissive + credential-free for the local build (the token
 * is a Bearer header, never a cookie); a deployed posture pins the origins (step-45 hardening).
 */
@Configuration
