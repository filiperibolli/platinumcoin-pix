...
| `GET` | `/internal/ledger/statement-window` | Service `ledger:read` | **Where the online statement ends and the archive begins** (step 53) → `{hotWindowDays, coldBefore}`. Published so payment-service can answer `422 USE_HOT_STATEMENT` on an export whose whole range is still online, instead of configuring a second copy of `pix.archive.hot-window-days` — one policy constant with two definitions is the shape of bug step 52 refused for clearing shards. It reads the **same** property and the same `Clock` the archiving job uses, so the boundary it reports is the one the job applies. Takes no account id: the window is the same for every customer, which is why a caller may cache it. |
...
| `jwt.internal-routes` | see `application.yml` | The per-route scope map: `POST /internal/ledger/postings` → `ledger:post`, `GET /internal/ledger/accounts/**`, `GET /internal/ledger/clearing-balance` and `GET /internal/ledger/statement-window` → `ledger:read`. First match wins; a route matching **nothing** is refused (an unscoped internal port is a configuration mistake, and the safe reading of a mistake on a money path is "no"). |
