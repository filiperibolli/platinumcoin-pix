 ...
 * disk, hence {@code Origin: null} — can call fraud-service from the browser. Copies the auth-service
 * pattern verbatim (see that class for the full rationale).
 *
 ...
 * {@code OPTIONS} is short-circuited instead of being rejected with {@code 401}. Present from the
 * skeleton (step 23) even though the only reachable surface is health, so step 24's {@code /score} card
 * works in the browser the moment it lands. Permissive + credential-free for the local build (the token
