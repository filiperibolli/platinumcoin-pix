 ...
 * from disk, hence {@code Origin: null} — can call payment-service from the browser. Copies the
 ...
 * {@code OPTIONS} is short-circuited instead of being rejected with {@code 401}. The send endpoint is
 * authenticated, so without the ordering the explorer could not reach it. Permissive + credential-free
 * for the local build (the token is a Bearer header, never a cookie); a deployed posture pins the
 * origins (step-45 hardening).
 */
@Configuration
