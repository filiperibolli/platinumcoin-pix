 ...
 * non-configurable connection-pool cap, which became the load-test throughput ceiling. SNS still goes
 * through LocalStack (see {@code SnsConfig}, which keeps using {@code endpointUrl}). Defaults to
 * {@code endpointUrl} ({@code ${DYNAMODB_ENDPOINT_URL:${aws.endpoint-url}}} in application.yml) so ITs
 * (which only override {@code aws.endpoint-url}) are unaffected.
 */
