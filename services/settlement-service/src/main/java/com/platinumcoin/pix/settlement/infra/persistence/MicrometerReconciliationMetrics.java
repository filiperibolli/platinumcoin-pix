 ...
 * {@code reconciliation.resolved} — {@code action=settled} and {@code action=reversed} — the reconciliation
 ...
 * {@code reconciliation.resolved{action}} exists at {@code 0} from boot — a funnel panel and an alert can
...
public class MicrometerReconciliationMetrics implements ReconciliationMetrics {
    ...
    private static final String METRIC = "reconciliation.resolved";
    ...
    public MicrometerReconciliationMetrics(MeterRegistry registry) {
        this.settled = Counter.builder(METRIC).tag("action", "settled").description(DESCRIPTION)
                .register(registry);
        this.reversed = Counter.builder(METRIC).tag("action", "reversed").description(DESCRIPTION)
                .register(registry);
    }
