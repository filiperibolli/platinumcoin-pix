...
| `CLEARING_SHARDS` / `pix.clearing-shards` | `16` | How many clearing sub-accounts exist (step 52). An **arrival** debits `SPI_CLEARING#NN` chosen by `CRC32(in-<endToEndId>) % N` — the same hot item outbound sends credit, from the other side, so sharding only the outbound half would leave it hot. A **reversal** ignores this value entirely: it debits the `clearingAccountId` recorded on the transaction, which is what makes changing N safe with payments in flight. Must match payment-service and ledger-service. |
