...
- Maven standard layout; package root `com.platinumcoin.pix.<service>`; inside: `api/` (controllers, DTOs), `domain/` (entities, services, ports), `infra/` (DynamoDB/SQS/Redis adapters, config). Hexagonal-lite: domain code never imports AWS SDK types.
