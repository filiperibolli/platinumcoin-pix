...
- Maven standard layout; package root `com.platinumcoin.pix.<service>`; inside: `api/` (inbound adapters — controllers, request/response records, exception mapping), `domain/` (entities & value objects, **`usecase/`**, domain exceptions, ports), `infra/` (outbound adapters — DynamoDB/SQS/SNS/Redis/HTTP — + Spring config). **Clean/hexagonal-lite per service (ADR-0010, amended by ADR-0011)** — pragmatic, not the full ceremony:
...
- **New-service checklist (the "scaffold" task of any skeleton step is not done until all of these exist).** Every scaffold step says only *"skeleton + Dockerfile + compose + README"*; that shorthand expands to:
  ...
  4. The three packages `api/` · `domain/` (+ `domain/usecase/`) · `infra/`, with **one `<Verb><Noun>UseCase` per inbound operation** and a `*BeansConfig` composition root (ADR-0011).
...
- Logging: **SLF4J everywhere** (never `System.out`, never a concrete logger API in code), structured JSON via the logback encoder. Every log line carries `correlationId` (+ `txId` when present) through MDC, and every meaningful stage of a flow logs at INFO with consistent event names — the explicit goal is that **one `correlationId` reconstructs the full path of a request across all services**. DEBUG for payloads (masked), WARN for retries/degradations, ERROR only for actionable failures.
  - **Call tracing lives at INFO — never require enabling DEBUG to see that a call happened.** Two layers, both mandatory:
    1. **Shared per-request line (free).** common-lib's `CorrelationIdFilter` emits one `INFO http.request method=… path=… status=… durationMs=…` per inbound call, inherited by every service with zero wiring (actuator paths skipped so healthchecks don't drown real calls). Do not re-implement it per service; do not remove it.
    2. **Business-stage lines (per service).** Every endpoint/handler logs its meaningful stages at INFO with a **consistent dotted event name** — `<domain>.<action>.<outcome>` (e.g. `account.me.lookup` → `account.me.resolved`, `auth.login.success`, `account.internal.miss`). A degradation is WARN (e.g. `account.me.missing` = valid token but no account); an actionable failure is ERROR.
  - **DEBUG is for adapter/payload detail only** (e.g. the raw DynamoDB `GetItem`/`Query` keys), off by default — the INFO layers above must already tell the full call story without it. **Every new service and every new endpoint follows this in the same step that introduces it** (same rule as the README/Postman/explorer conventions).
