...
| Replay: impossible (consumed = gone); recovery path is the reconciliation loop + S3 audit trail | Replay is native: rewind the group's offset, or spin a new group from offset 0 within retention | Kafka retention turns the topic itself into a short-term event store; our equivalent long-term store is the S3 audit log (step 37). |
