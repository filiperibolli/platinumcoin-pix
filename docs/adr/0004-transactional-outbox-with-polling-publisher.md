...
**Status:** Accepted (supersedes the initial Streams-based draft) · **Date:** 2026-07-02 · **Amended by:** [ADR-0019](0019-outbox-lanes-and-priority.md) (step 71)

> **Amended by ADR-0019 — the guarantee stands, the capacity did not.** Everything below about the
> *write* (one `TransactWriteItems`, no dual write) and about the *direction of failure*
> (publish-then-mark ⇒ at-least-once) is unchanged and was never in question. What ADR-0019 changes is
> that **one** sparse index drained by **one** thread is one FIFO, so any event type could delay any
> other: `docs/load/RESULTS.md` Context 2 records a correct payment `REVERSED` because its `PixDebited`
> queued behind 55,538 events that had **no subscriber at all**. Since step 71 the drain is partitioned
> into `settlement` / `notification` / `audit` lanes — `gsi3pk` is `OUTBOX#UNPUBLISHED#<LANE>`, one
> publisher and one lag SLO per lane. Read the two paragraphs below on ordering with that in mind.
...
2. **Delivery (polling publisher)**: a scheduled component in payment-service (every 1–2s) queries the **sparse GSI3** (`gsi3pk = OUTBOX#UNPUBLISHED`, `gsi3sk = occurredAt`; **lane-scoped to `OUTBOX#UNPUBLISHED#<LANE>` with one publisher per lane since ADR-0019**) oldest-first, publishes each event to SNS topic `pix-events` (message attributes `eventType`/`eventId` for subscription filtering), then removes the `gsi3pk` attribute — the item drops out of the sparse index, marking it published. Crash between publish and mark ⇒ republish on the next tick; delivery is **at-least-once**, and every consumer dedupes by `eventId` (house rule).
...
- Ordering: the poll processes oldest-first, but strict per-transaction ordering across redeliveries is not guaranteed — consumers rely on guarded status transitions, not event order (already required by at-least-once). **ADR-0019 makes that explicit rather than accidental**: oldest-first is preserved *within* a lane (the sort key is untouched), cross-lane ordering is stated as not guaranteed, and with `max-in-flight > 1` a lane's batch is *claimed* oldest-first while concurrent publishes may reach the broker in either order — none of which takes away a guarantee this ADR ever offered.
