...
The public read model over the internal ledger query: payment-service is the single public edge, so the statement (like balance) is exposed here, not directly from ledger-service. You'll pass the **opaque cursor** straight through (base64 `LastEvaluatedKey`), format money at the edge (cents → decimal string), and mask counterpart display (don't leak internal account ids). This keeps the OpenAPI contract stable while the storage detail (single-table ledger, cold archive later) stays hidden.
...
3. Clamp `limit` (default 20, max 100); reject tampered cursors (400).
