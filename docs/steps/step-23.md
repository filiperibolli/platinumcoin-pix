...
3. Scaffold `services/fraud-service` (skeleton + Dockerfile + compose entry + `README.md`, port 8083) — expands per the **new-service checklist** in `CLAUDE.md`: the three packages incl. `domain/usecase/` (one `<Verb><Noun>UseCase` per inbound operation, ADR-0011), a `*BeansConfig` composition root, and the `*ArchitectureTest` with **both** ArchUnit rules from day one. Depends on common-lib and a Redis client.
