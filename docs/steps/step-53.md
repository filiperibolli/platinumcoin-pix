...
## Prerequisites
Steps 41 (hot statement API), 43 (cold archive populated).
...
## Tests (TDD)
- Happy path IT: seed archive for 3 months → request export → poll to READY → download CSV → row count and totals match.
...
- Ownership: token for another account ⇒ 404.
...
- [ ] Hot-window requests steered to the hot API, not silently duplicated

## CHANGELOG entry
`### Added` → `Async cold statement export: 202 + polling status URL + presigned CSV artifact from the S3 archive (step 53)`
