...
**Invariant (checkable at any time):** `Σ balanceCents over all accounts (including SPI_CLEARING and every SPI_CLEARING#NN shard) = Σ of initial seeds` — postings move money, never create or destroy it. The invariant test suite (step 15) asserts this under a concurrent debit storm.
...
**System accounts:** `ACCOUNT#SPI_CLEARING` **and its write shards `ACCOUNT#SPI_CLEARING#00..#15`** (money in flight to/from BACEN — exempt from the `balance >= x` condition, since its balance represents an inter-bank position and may go negative on inbound-heavy days; the exemption is a **prefix** match in `AccountPolicy`, so every shard inherits it); `ACCOUNT#SPI_SETTLED` (step 33 — money that has **settled out** to the SPI network, the credit counterpart of a `CLEARING_RELEASE` so that a settlement draws clearing down while Σ over all accounts stays **zero**; seeded at 0, only ever credited, so it never needs the exemption); and `ACCOUNT#SEED` (initial funding source for demo users — **also exempt**: its balance is negative by construction, the double-entry counterpart of the seeded user balances, so Σ over all accounts nets to **zero**). **Write sharding (step 52):** at 500 TPS all external sends hit the single clearing item, so it is sharded into `SPI_CLEARING#00..#15` by `CRC32(txId) % CLEARING_SHARDS` (default 16; `1` reproduces the un-sharded account). The shards are created by `05-seed-ledger.sh` at 0 — they **must** exist before the first payment, because the credit leg is conditioned on `attribute_exists(pk)` and an absent shard is a refused posting on the money path, not a silent zero. All at 0 ⇒ Σ over the table is still exactly 0 whatever N is. The bare `SPI_CLEARING` item is kept alongside them: `CLEARING_SHARDS=1` uses it, and the clearing position sums it too, so money parked there before N was raised stays visible. A reversal/release must hit the **same** shard the debit credited, which is why the shard used is persisted on the transaction (§4, `clearingAccountId`) and read back rather than re-derived. The logical position is `GET /internal/ledger/clearing-balance` (sum + per-shard breakdown). **Changing N with payments in flight:** raising it is safe (the sum still enumerates the bare `SPI_CLEARING`, so money credited under the old map stays visible, and every in-flight payment reverses against the id recorded on its transaction); **lowering** it is not — money in a no-longer-configured shard still reverses correctly but drops out of the sum without appearing in `missingAccounts`, so drain clearing to zero first.
...
- `clearingAccountId` (step 33, external send only) — the **exact** clearing account the acceptance-time
  ...
  reversal debits the same account it credited. Since step 52 that is one of `SPI_CLEARING#00..#15`, and a
  reversal that re-derived the shard instead of reading the one used would drain the wrong sub-account and
  break the per-shard balance — while staying perfectly balanced and leaving global Σ untouched, which is
  why the invariant suite asserts per-shard sums and not only the global one. It is also what makes
  `CLEARING_SHARDS` safe to change with payments in flight. An internal send never touches clearing, so
